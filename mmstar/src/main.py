...
def T5Trainer(
    ...
    if trainer.is_world_process_zero():
        ...
        for idx, qid in enumerate(test_qids):
            ...
            print("results rationale",results_rationale[str(qid)])
            print("results_reference",results_reference[str(qid)])
        scores = get_scores(results_ans, results_rationale, results_reference, os.path.join(args.data_root, "problems.json"))
        print("scores",scores)
        preds = [pred.strip() for pred in preds]
        output_data = {
                "num_fail": num_fail,
