...
def load_data_std(args):
    ...
    train_qids = pid_splits['%s' % (args.train_split)][:5]
...
def load_data_img(args):
    ...
    pid_splits = json.load(open(os.path.join(args.data_root, 'pid_splits.json')))
    captions = json.load(open(args.caption_file))["captions"]
    name_maps = json.load(open('/mnt/mm-cot/mmstar_data/name_map.json'))
      
    # check
    if args.img_type == "resnet":
        image_features = np.load('/mnt/mm-cot/vision_features/resnet.npy')
        ...
        image_features = np.load('/mnt/mm-cot/vision_features/clip.npy')
    elif args.img_type == "detr":
        image_features = np.load('/mnt/mm-cot/vision_features/detr.npy')
    elif args.img_type == "vit":
        image_features = torch.load("/mnt/mm-cot/mmstar_data/vision_features/vit.pth")
    else:
        image_features = np.load('/mnt/mm-cot/vision_features/detr.npy')
   ...
   # train_qids = pid_splits['%s' % (args.train_split)][:5] 
    ...
    print(f"number of test problems: {len(test_qids)}\n")
    qids = {'train': train_qids, 'val':val_qids,'test':test_qids}
    # Convert problems to a DataFrame
   # problems_df = pd.DataFrame.from_dict(problems, orient='index')

    # Find the QIDs for Science and Technology
   # science_qids = problems_df[problems_df['category'] == 'science & technology'].index.tolist()

    # Now overwrite val_qids and test_qids with ONLY science and tech questions
   # val_qids = [qid for qid in val_qids if qid in science_qids]
   # test_qids = [qid for qid in test_qids if qid in science_qids]

   # Update the qids dictionary as well
    #qids['val'] = val_qids
    #qids['test'] = test_qids
    #qids = {'train': train_qids, 'val':val_qids,'test':test_qids}
    #print(f"number of SCIENCE val problems: {len(val_qids)}\n")
    #print(f"number of SCIENCE test problems: {len(test_qids)}\n")
